...
export default function CartPage() {
  ...
  function handleCheckout() {
    ...
    let message = "👋 Bonjour l'équipe AYVER ! Je souhaite valider ma commande :\n\n";

    cart.forEach((item) => {
      const tailleInfo = item.size ? ` (Taille: ${item.size})` : "";
      message += `▪️ ${item.qty}x ${item.name}${tailleInfo} - ${item.price * item.qty} DH\n`;
    });

    message += `\n*Sous-total :* ${total} DH\n`;
    message += `*Livraison :* Gratuite au Maroc 🇲🇦\n`;
    message += `*Total à payer : ${total} DH*\n\n`;
    message += `Merci de me confirmer la disponibilité et la livraison !`;

    const encodedMessage = encodeURIComponent(message);
    const adminPhone = "212694434456";
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-checkout { width: 100%; margin-top: 30px; padding: 18px; background: var(--dark); color: var(--cream); border: none; cursor: pointer; font-family: 'Jost', sans-serif; letter-spacing: 3px; font-size: 12px; text-transform: uppercase; font-weight: 500; transition: background 0.3s; }
        ...
        @media(max-width:1024px){ .features-inner{grid-template-columns:repeat(2,1fr)} .footer-inner { flex-direction:column; text-align:center; } .footer-links { justify-content:center; } }
        @media(max-width:640px){ .features-inner{grid-template-columns:1fr 1fr} .nav-links{display:none;} }
      `}} />
      ...
      <div className="cart-container">
        <div className="cart-header">
          ...
          <p>Révisez vos articles AYVER avant la validation finale via WhatsApp</p>
        </div>

        {cart.length === 0 ? (
          <div className="cart-empty">
            Votre vestiaire est actuellement vide.
          </div>
        ) : (
          <>
